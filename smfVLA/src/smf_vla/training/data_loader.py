"""
libero data loader for pi0.5 action-expert training (shared by smfVLA/snapflow/freeflow).
...
def create_data_loader(
    ...
    # ── video path index (v2.1 only) — build before frame_entries so we can filter ──
    ...
    if is_video_format:
        ...
        logger.info(f"Video episodes available: {len(episode_video_paths)}")
