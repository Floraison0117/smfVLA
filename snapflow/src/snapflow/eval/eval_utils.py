...
# ── 路径常量 ──────────────────────────────────────────────
# Use PROJECT_ROOT env var if set, otherwise compute from file location (go up to project root)
# This file is at src/snapflow/eval/eval_utils.py, so we need to go up 4 levels to reach project root
_PROJECT_ROOT_FROM_FILE = pathlib.Path(__file__).resolve().parent.parent.parent.parent
PROJECT_ROOT = pathlib.Path(os.environ.get("PROJECT_ROOT", str(_PROJECT_ROOT_FROM_FILE)))
