...
提供加载SnapFlow模型checkpoint的接口，用于LIBERO评估。
"""
...
SNAPFLOW_DIR = PROJECT_ROOT / "snapflow"


def setup_snapflow_paths():
    """设置SnapFlow模型加载所需的Python路径。"""
    import sys
    paths_to_add = [
        str(SNAPFLOW_DIR / "src"),
        str(OPENPI_DIR / "src"),
...
def load_snapflow_config(
    ...
    Args:
        ...
        alpha: Flow matching alpha参数
        lambda_consistency: Consistency loss权重

    Returns:
        Pi05SnapFlowConfig对象
    """
    setup_snapflow_paths()

    from snapflow.models.pi05_snapflow import Pi05SnapFlowConfig

    return Pi05SnapFlowConfig(
        pi05=True,
...
def load_snapflow_model(
    ...
    """
    加载SnapFlow模型并构建Policy。

    Args:
        checkpoint_dir: SnapFlow checkpoint目录路径
        nfe: Number of function evaluations (1 for SnapFlow)
    ...
    logger.info(f"Loaded {loaded_count} parameters for SnapFlow model")
...
def load_snapflow_config_dict(checkpoint_dir: str) -> dict:
    """
    从SnapFlow checkpoint目录加载配置字典（如果存在）。
